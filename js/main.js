...
(function () {
  ...
  var wrappers = Array.prototype.slice.call(
    track.querySelectorAll(".orbit-card-wrapper"),
  );
  if (!wrappers.length) {
    return;
  }
  ...
  wrappers.forEach(function (wrapper) {
    ...
    var supports = Array.prototype.slice.call(
      wrapper.querySelectorAll(".orbit-card-support-visual"),
    );
    cardBgs.push(bg);
    visuals.push(visual);
    supportVisuals.push(supports);
    supportTweenSetters.push(
      supports.map(function (support) {
        if (!support) {
          return null;
        }
        return {
          x: gsap.quickTo(support, "x", {
            duration: SUPPORT_SMOOTH,
            ease: SUPPORT_TWEEN_EASE,
          }),
          y: gsap.quickTo(support, "y", {
            duration: SUPPORT_SMOOTH,
            ease: SUPPORT_TWEEN_EASE,
          }),
          scale: gsap.quickTo(support, "scale", {
            duration: SUPPORT_SMOOTH,
            ease: SUPPORT_TWEEN_EASE,
          }),
          opacity: gsap.quickTo(support, "opacity", {
            duration: SUPPORT_SMOOTH,
            ease: SUPPORT_TWEEN_EASE,
          }),
        };
    ...
    bgOpacitySetters.push(
      bg
        ? gsap.quickTo(bg, "opacity", { duration: DIM_SMOOTH, ease: DIM_EASE })
        : null,
    );
    visualOpacitySetters.push(
      visual
        ? gsap.quickTo(visual, "opacity", {
            duration: DIM_SMOOTH,
            ease: DIM_EASE,
          })
        : null,
  ...
  function measureSupportOffsets() {
    ...
    supportVisuals.forEach(function (supports) {
      ...
      supports.forEach(function (support) {
        ...
        var popX = readCssNumber(
          support,
          "--support-pop-x",
          SUPPORT_X_MAX_DEFAULT,
        );
        var popY = readCssNumber(
          support,
          "--support-pop-y",
  ...
  function updateOrbitClearance() {
    ...
    var messageEl = workSection
      ? workSection.querySelector(".message_wrapper")
      : null;

    var titleGap = readCssNumber(orbitWrapper, "--orbit-title-gap", 0);
    var titleBuffer = readCssNumber(orbitWrapper, "--orbit-title-buffer", 0);
    var topAdjust = readCssNumber(
      orbitWrapper,
      "--orbit-overflow-top-adjust",
      0,
    );
    var messageGap = readCssNumber(orbitWrapper, "--orbit-message-gap", 0);
    var bottomAdjust = readCssNumber(
      orbitWrapper,
      "--orbit-overflow-bottom-adjust",
      0,
    );
    var bottomCap = readCssNumber(
      orbitWrapper,
      "--orbit-overflow-bottom-cap",
      NaN,
    );
    var desiredTop = titleEl
      ? getLayoutBottom(titleEl) + titleGap + titleBuffer
      : viewportRect.top;
    var desiredBottom = messageEl
      ? getLayoutTop(messageEl) - messageGap
      : viewportRect.bottom;

    var overflowTop = Math.max(
      0,
      desiredTop - bounds.minTop + supportTopBuffer + topAdjust,
    );
    var rawOverflowBottom =
      bounds.maxBottom + supportBottomBuffer - desiredBottom;
    ...
    orbitWrapper.style.setProperty(
      "--orbit-overflow-top",
      Math.ceil(overflowTop) + "px",
    );
    orbitWrapper.style.setProperty(
      "--orbit-overflow-bottom",
      Math.ceil(overflowBottom) + "px",
    );
  }
  ...
  function update(instant) {
    ...
    wrappers.forEach(function (wrapper, i) {
      ...
      if (supports && supports.length) {
        ...
        var supportScale = gsap.utils.interpolate(
          SUPPORT_SCALE_MIN,
          SUPPORT_SCALE_MAX,
          easedFocus,
        );
        supports.forEach(function (support, supportIndex) {
          var supportPopX =
            supportPopXs[i] && supportPopXs[i][supportIndex] !== undefined
              ? supportPopXs[i][supportIndex]
              : SUPPORT_X_MAX_DEFAULT;
          var supportPopY =
            supportPopYs[i] && supportPopYs[i][supportIndex] !== undefined
              ? supportPopYs[i][supportIndex]
              : SUPPORT_Y_MAX_DEFAULT;
          var supportX = gsap.utils.interpolate(
            SUPPORT_X_MIN,
            supportPopX,
            easedFocus,
          );
          var supportY = gsap.utils.interpolate(
            SUPPORT_Y_MIN,
            supportPopY,
            easedFocus,
          );
          var supportOpacity = gsap.utils.interpolate(
            SUPPORT_OPACITY_MIN,
            SUPPORT_OPACITY_MAX,
            easedFocus,
          );
          var setters =
            supportTweenSetters[i] && supportTweenSetters[i][supportIndex];
  ...
  function updateTitle(index) {
    var nextTitle =
      TITLES[index] || (titleTextEl ? titleTextEl.textContent : "");
    var nextImage =
      TITLE_IMAGES[index] ||
      (titleImageEl ? titleImageEl.dataset.titleSrc || "" : "");
    var currentTitle = titleTextEl ? titleTextEl.textContent : "";
    var currentImage = titleImageEl ? titleImageEl.dataset.titleSrc || "" : "";
    if (
      currentTitle === nextTitle &&
      (!titleImageEl || currentImage === nextImage)
    ) {
      return;
    }
  ...
  function centerIndex(index) {
    ...
    gsap.to(track, {
      ...
      onUpdate: function () {
        dragX =
          gsap.getProperty(track, "x") - (centerOffset() - index * step());
        update();
        scheduleClearance();
      },
    });
  ...
  if (typeof ResizeObserver !== "undefined") {
    ...
    var heroTitle = orbitWrapper
      ? orbitWrapper.querySelector(".orbit_hero_title")
      : null;
    if (heroTitle) {
